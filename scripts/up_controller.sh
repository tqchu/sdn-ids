...
consumer_container_id=$(docker run -d --name ml_detector   --network sdn-ids_default   -v $(pwd)/flows:/app/flows   -e KAFKA_BOOTSTRAP_SERVERS=kafka:29092   -e PUSHGATEWAY_URL=pushgateway:9091   -e LOKI_URL=http://loki:3100 -e CONTROLLER_HOST=172.17.0.1 -e DPID=0000000c29bb448a ml-detector)
